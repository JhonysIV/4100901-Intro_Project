...
int main(void)
{
  ...
  char *welcome_msg = "Tranquilos falsa alarma\r\n";
...
/*
  ...
  if (HAL_GetTick() - last_heartbeat_time >= 100) // Cada 500 ms
  {
    ...
    last_heartbeat_time = HAL_GetTick(); // se puede borrar esta parte para hacer que el led solo encienda con el boton y no parpadie
  }
*/
  ...
  if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
  {
    ...
    HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET);//apaga LED LD2 
    // 2. Enviar mensaje por UART (opcional)
    sprintf(tx_buffer, "Ya No\r\n");
    HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
